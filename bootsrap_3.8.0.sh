...
# Copy the folder containing the IdentifyLoopUnrolling pass to LLVM source tree.
cd ../..
cp -r IdentifyLoopUnrolling llvm-RS-3.8.0/llvm-3.8.0.src/lib/Transforms/.
sed -i.bak 's/^\(PARALLEL_DIRS = .*\)/\1 IdentifyLoopUnrolling/' llvm-RS-3.8.0/llvm-3.8.0.src/lib/Transforms/Makefile
echo "add_subdirectory(IdentifyLoopUnrolling)" >> llvm-RS-3.8.0/llvm-3.8.0.src/lib/Transforms/CMakeLists.txt 
